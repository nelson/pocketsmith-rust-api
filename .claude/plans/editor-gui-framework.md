...
> [`editable-rules-ui.md`](./editable-rules-ui.md) Â§3. The keystone GUI PR:
...
first stage wired is **prefix + suffix** (loop stages â capture flags +
reorder â the hardest shape, per the master plan), so the framework is
proven against the worst case.

- **PR-A â editor core**: two-column detail (rule list + editor card),
...
- **PR-B â activity log + dirty banner**: the newest-first capped-100
  rule-change log in the activity panel; the `â  N payees would re-stage`
  banner via `would_restage`; `/pipeline/rescan` runs `scan` + clears it.
- **PR-C â per-rule impact cache**: the `rule_impact` table, the scan
  attribution pass, and the cached `"412 txns Â· $8.4k"` impact column in
...
**prefix + suffix** stages first (loop stages â capture flags + reorder â
the hardest shape), exactly as the master plan locks in; the other six
stages follow in their own PRs (Â§4 of the master plan).

All interactions are HTMX fragments POSTed under `/pipeline/`, no
client-side rule state. EditâEvaluateâSave split: Evaluate (POST, carries
...
**Library (`src/rules/`)**
- `src/db/schema.rs` â add `rule_impact` cache table (+ `IF NOT EXISTS`).
- `src/normalise/scan.rs` â fold a `rule_impact` refresh pass into `scan`.
- `src/rules/impact/` â add an attribution helper that maps each distinct
  payee â the rule id that won its stage (reused by the scan pass).

**Serve (`src/bin/serve/`)**
- `src/bin/serve/main.rs` â register the new `/pipeline/*` routes.
- `src/bin/serve/pipeline/mod.rs` â declare new submodules.
- `src/bin/serve/pipeline/views.rs` â two-column detail; rule list with
  cached impact column; wire selection + active-rule state.
- `src/bin/serve/pipeline/editor.rs` â **new**: parameterised editor card
  (edit / evaluate / new) driven by a per-stage field descriptor.
- `src/bin/serve/pipeline/impact.rs` â **new**: HTML rendering of the four
  (first-match) / two (loop) `compute_buckets` outcome buckets.
- `src/bin/serve/pipeline/handlers.rs` â **new**: create / edit / delete /
  reorder / evaluate / re-scan handlers.
- `src/bin/serve/state.rs` â add a pipeline rule-change activity log + the
  active-rule pointer.
- `src/bin/serve/css.rs` â editor/impact/rule-list classes (port from the
  mockups' inline CSS).
- `src/bin/serve/js.rs` â pipeline keyboard shortcuts (A/E/N/Y/B, Alt+â/â).

## Reuse (already built â do NOT reimplement)

- `rules::commit(conn, &Mutation, DumpPolicy::Background{db_path}, Some(&cache))`
  â the single atomic save seam (`src/rules/commit.rs`). Invalidates the
...
- `rules::validate::{validate_draft, StageSchema}` â field schema + the
  one validator. The editor form reads `StageSchema::for_stage`.
- `rules::activity::RuleChange::describe(&Mutation, before)` â the
  `+ added` / `~ edited` / `â deleted` / `moved` activity line.
- `rules::dirty::would_restage(conn)` â the "N payees would re-stage" count.
- `rules::crud::{get, list, is_movable, insert_rule, update_rule,
  delete_rule, move_rule}` + `model::{Mutation, RuleData, MoveTarget}`.
- `crate::render::render_page_with_chips` â page shell (`#queue`/`#detail`/
  `#activity`). `freshness::header_chips` for the chips.
- `normalise::scan::scan(conn)` â the re-scan the banner triggers.
- The CLI bucket renderer (`src/bin/rule/render.rs::evaluate`) â reference
...
| Method | Path | Returns |
|--------|------|---------|
| GET  | `/pipeline/stage/<slug>` | stage detail (rule list + empty editor) â exists, extend to 2-col |
...
routes are all GET/param-based) â add a small `application/x-www-form-urlencoded`
...
- [x] **Impact rendering** (`impact.rs`): render `Buckets` (2 buckets for
      ...
      stages) + tester result string per the mockup (counts, â¤6 samples,
      "show all", `was: X â Y`). Pure-fn unit tests over fixture buckets.
...
- [x] **Keyboard** (`js.rs`): A/E/N/Y/B + Alt+â/â reorder; extend the `?`
      hints overlay for the Pipeline tab.
- [x] **Integration test (PR-A)**: create â list shows it â evaluate shows
      buckets â save â mutation re-dumps `rules/<stage>.sql`; reorder a
      prefix changes pipeline output for an affected payee.

### PR-B â activity log + dirty banner
...
- [x] **Dirty banner + re-scan** (`views.rs` + `handlers.rs`):
      `â  N payees would re-stage Â· re-scan now â»` via `would_restage`;
...
- `cargo test` (library) and `cargo test --features web` (serve) â all green.
...
- `git diff rules/*.sql` after edits is human-reviewable (the Â§6 guarantee).

## Decisions (resolved with reviewer)

1. **Scope** â split the keystone into three PRs (A: editor core; B:
   activity log + dirty banner; C: per-rule impact cache).
2. **First stage wired** â prefix + suffix (loop stages), per the master
   plan; the other six stages follow in their own PRs (master plan Â§4).
