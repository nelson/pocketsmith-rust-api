# Writeback Stage 3 — Expand to remaining locally-mutated fields

> See [writeback-overview.md](./writeback-overview.md) for the rollout context. Do not begin this stage until the [Stage 2](./writeback-stage-2-observation.md) debrief is committed.

## Context

[Stage 1](./writeback-stage-1-transfer-pair-mvp.md) only pushes `is_transfer` + `category_id` from `transfers --apply`. The other place we write locally today is `normalise`, which mutates `payee` (and maybe other fields — verify). Stage 3 generalises Stage 1's pending-pull-PUT-mark loop to handle every locally-mutated field, still gated by the Stage 1 timestamp guard.

## Scope

- Drop Stage 1's `(_mask & 16) != 0` and `reason='transfers'` filters from the pending query.
...
- All Stage 1 safety still applies: timestamp guard, `_pushed_at` marking, `_writeback_log` row per attempt.
...
WHERE h._mask != 0
  AND l.reason NOT IN ('pocketsmith','writeback')
  AND h._pushed_at IS NULL
  AND EXISTS (SELECT 1 FROM transactions t WHERE t.id = h._rowid);
```
...
- **edit** `src/writeback/mod.rs` — generalise pending query, generalise body builder, generalise mark-as-pushed (covers all involved `_version`s).
...
15. Pending query ignores `reason='pocketsmith'` and `reason='writeback'`.
16. Two unpushed history rows on the same txn (one normalisation, one transfers) → single PUT with both fields set.
17. After a successful Stage-3 PUT, all involved `_version`s have `_pushed_at` set.
...
4. **Which fields are actually mutated locally today?** Grep for `with_transaction_change_log` callers / `UPDATE transactions` to enumerate. Today: `transfers` (`is_transfer` + `category_id`), `normalise` (`payee`, and possibly more — confirm). Anything else?
