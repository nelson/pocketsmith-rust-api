# Writeback Stage 2 — Observation (no code)

> See [writeback-overview.md](./writeback-overview.md) for the rollout context. This stage gates Stages 3 and 4: real-world usage of [Stage 1](./writeback-stage-1-transfer-pair-mvp.md) determines what we build next.

## Context

Stage 1 ships the bluntest possible writeback. Stage 2 is the gate that decides what Stages 3 and 4 actually need to do. We do not want to design conflict detection (Stage 4) or expand to more fields (Stage 3) until we have seen Stage 1 behave in anger against real Pocketsmith data over real user activity.

## Scope

Use Stage 1 in production for a defined observation window. Read `_writeback_log` (and the live API where needed). Produce a written debrief that explicitly answers: "what should Stage 3 and Stage 4 actually look like?"

## Activities

1. Run `writeback` daily (or after each `transfers --apply` session) over the observation window.
2. After each run, inspect `_writeback_log`:
   - `SELECT outcome, COUNT(*) FROM _writeback_log GROUP BY outcome` — baseline counts.
   - `SELECT * FROM _writeback_log WHERE outcome='skipped_changed_upstream'` — every false-positive timestamp guard candidate.
...
A short debrief committed at `plans/writeback-stage-2-debrief.md` (NEW — written after the observation window). Sections:
...
3. **Tooling.** Ad-hoc SQL only, or build a tiny `writeback-report` binary that summarises `_writeback_log`? Lean ad-hoc to start; promote to a binary only if we find ourselves running the same three queries every day.
