...
/// Entry point for the `rule` subcommand. `args` are the tokens *after*
/// the `rule` verb (e.g. `["list", "--stage", "merchants"]`).
...
fn run(args: &[String]) -> Result<(), AppError> {
    ...
    match verb {
        ...
        other => Err(AppError::usage(format!("unknown verb {other:?}. Try `pocketsmith rule --help`."))),
    }
}
...
\n\
USAGE\n  \
  pocketsmith rule list   --stage <stage> [--json]\n  \
  pocketsmith rule show   --stage <stage> --id <id> [--json]\n  \
  pocketsmith rule test   --stage <stage> [candidate flags] \"<string>\"\n  \
  pocketsmith rule add    --stage <stage> [field flags] [--apply|-a] [--json]\n  \
  pocketsmith rule edit   --stage <stage> --id <id> [field flags] [--apply|-a] [--json]\n  \
  pocketsmith rule rm     --stage <stage> --id <id> [--apply --force | -af] [--json]\n  \
  pocketsmith rule move   --stage <stage> --id <id> (--before <id> | --after <id>) [--apply|-a]\n\
\n\
STAGES\n  \
