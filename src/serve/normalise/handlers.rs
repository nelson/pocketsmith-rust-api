...
//!
//! Mirrors `crate::serve::handlers` (transfers tab) in shape: each action records
...
use crate::serve::tab::next_after;
...
/// `handle_action` in `crate::serve::handlers`.
...
/// counter. Mirrors `handle_undo` in `crate::serve::handlers`.
...
/// `handle_clear_all_skipped` in `crate::serve::handlers`.
