...
//! handlers, views, and the JS in [`crate::serve::js`].
...
fn transfers_page_renders_tab_bar_and_queue_and_actions() {
    ...
    let html = crate::serve::transfers::views::render_page_shell(&state).into_string();
...
fn normalise_page_renders_tab_bar_and_queue_and_actions_and_trace() {
    let state = fresh_state();
    let html = crate::serve::normalise::views::render_page_shell(&state).into_string();
...
fn transactions_page_renders_tab_bar_and_three_panes() {
    let state = fresh_state();
    let html = crate::serve::transactions::views::render_page_shell(&state).into_string();
...
fn transactions_detail_panel_renders_action_buttons_on_norm_pending_card() {
    ...
    let html = crate::serve::transactions::views::render_detail_fragment(&state, 50).into_string();
...
fn transactions_activity_panel_shows_recent_decisions_with_undo_btn() {
    ...
    crate::serve::transactions::handlers::act_norm(&state, 60, crate::serve::state::Decision::Confirm);
...
fn transactions_queue_emoji_is_clickable_to_undo_when_decided() {
    ...
    crate::serve::transactions::handlers::act_norm(&state, 61, crate::serve::state::Decision::Confirm);
...
fn transactions_detail_renders_pipeline_trace_and_siblings() {
    ...
    let html = crate::serve::transactions::views::render_detail_fragment(&state, 70).into_string();
    contains_all(
        &html,
        &[
...
fn categorise_page_renders_tab_bar_queue_detail_and_actions() {
    use pocketsmith::db::category_proposals::{self as cp, CategoryProposalRow};
    use pocketsmith::review::Status;
    ...
    let html = crate::serve::categorise::views::render_page_shell(&state).into_string();
    contains_all(
        &html,
        &[
            "class=\"tab-bar\"",
