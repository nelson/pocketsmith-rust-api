...
pub fn render_page_shell(state: &Arc<Mutex<AppState>>) -> Markup {
    ...
    let chips = crate::serve::freshness::header_chips(&st.conn);
    crate::serve::render::render_page_with_chips(
        "transactions",
        "Transactions",
        chips,
        queue,
...
fn render_active_detail(
    ...
    if let Some(v) = views.iter().find(|x| x.row.id == id) {
        ...
        let siblings = v
            ...
            .original_payee
            .as_deref()
            .map(|op| crate::serve::normalise::helpers::matching_transactions(&state.conn, op))
            .unwrap_or_default();
...
fn count_decisions(activity: &[crate::serve::state::TxnActivityEntry], d: Decision) -> usize {
...
mod tests {
    use super::*;
    use crate::serve::transactions::helpers::TxnQueueRow;
...
pub fn render_detail_fragment(state: &Arc<Mutex<AppState>>, txn_id: i64) -> Markup {
    ...
    let siblings = row
        .original_payee
        .as_deref()
        .map(|op| crate::serve::normalise::helpers::matching_transactions(&st.conn, op))
        .unwrap_or_default();
    render_detail(&row, pair, norm, cat, pipeline.as_ref(), &siblings)
}
...
fn render_detail(
    ...
    siblings: &[crate::serve::normalise::helpers::MatchingTxn],
) -> Markup {
    ...
    html! {
        ...
        @if let Some(p) = pipeline { (crate::serve::trace::render_pipeline_trace(p)) }
