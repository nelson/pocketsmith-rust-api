...
use pocketsmith::db::category_proposals::CategoryProposalRow;
use pocketsmith::review::Status;

use crate::serve::state::{AppState, Decision};
use crate::serve::tab::count_decisions;
...
pub fn render_detail_fragment(state: &Arc<Mutex<AppState>>, key: &str) -> Markup {
    ...
    match pocketsmith::db::category_proposals::get(&st.conn, key) {
        Ok(Some(row)) => {
...
fn render_full_page(
    ...
    let chips = crate::serve::freshness::header_chips(&state.conn);
...
fn render_detail(row: &CategoryProposalRow, title: Option<&str>, is_skipped: bool) -> Markup {
    ...
    html! {
        ...
        (crate::serve::render::render_actions(&action_base, is_skipped))
    }
}
