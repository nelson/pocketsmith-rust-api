...
mod tests {
    ...
    fn insert_txn_v2(conn: &Connection, id: i64, original_payee: &str, payee: &str) {
        // INSERT into transactions fires _transaction_changes_insert which
        // requires _current_operation, so wrap each test insert in a
        // with_operation("test-seed", ...) block.
