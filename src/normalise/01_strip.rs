...
pub(crate) fn suffix_patterns() -> &'static [StripPattern] {
    ...
    PATTERNS.get_or_init(|| {
        let patterns: Vec<(&str, &'static str)> = vec![
            ...
            (r"\s+(?P<location_raw>NSWAU)$", "NSWAU suffix"),
            (r"\s+(?P<location_raw>NS) AUS$", "NS AUS suffix"),
            (r"\s+(?P<location_raw>AU) AUS$", "AU AUS suffix"),
            (r"\s+(?P<location_raw>AUS)$", "AUS suffix"),
            (r"\s+(?P<location_raw>AU)$", "AU suffix"),
            (r"\s+(?P<location_raw>NLD)$", "NLD suffix"),
            (r"\s+(?P<location_raw>SGP)$", "SGP suffix"),
            (r"\s+(?P<location_raw>USA)$", "USA suffix"),
            (r"\s+(?P<location_raw>IDN)$", "IDN suffix"),
            (r"\s+(?P<location_raw>GBR)$", "GBR suffix"),
            ...
            (r"\s+(?P<location>(?:NSW|VIC|QLD|WA|SA|TAS|ACT|NT)\s+\d{4,6})$", "State + postcode"),
            (r"\s+(?P<location>(?:NSW|VIC|QLD|WA|SA|TAS|ACT|NT))$", "State suffix"),
        ];
        patterns
            .into_iter()
