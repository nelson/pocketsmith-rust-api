...
use pocketsmith::categorise::apply;
use pocketsmith::categorise::places::{self, GooglePlacesClient};
use pocketsmith::categorise::scan;
use pocketsmith::db;
use pocketsmith::db::category_proposals as cp;
use pocketsmith::review::Status;

/// `pocketsmith categorise` — the final pipeline stage: assign Pocketsmith
/// categories + labels to merchant transactions, sourced from Google Places.
///
/// Verbs:
...
pub fn run(args: &[String]) -> Result<()> {
    ...
    match cmd {
        ...
        other => {
            bail!("unknown subcommand: {other}. Try `pocketsmith categorise --help`.");
        }
    }
}
...
fn print_help() {
    println!(
        "usage:\n  \
         pocketsmith categorise scan                 scan merchant txns, stage proposals\n  \
         pocketsmith categorise list [--status S]    list proposals (S = pending|confirmed|rejected); --json\n  \
         pocketsmith categorise confirm <key|--all>  confirm a proposal (or all pending)\n  \
         pocketsmith categorise reject  <key>        reject a proposal\n  \
         pocketsmith categorise apply                write confirmed category+labels to transactions\n  \
         pocketsmith categorise lookup \"<query>\"      ad-hoc Places probe (fills the cache); --json\n\n\
