//! `pocketsmith normalise` — scan/apply review flow for payee normalisations.
//!
...
fn print_help() {
    println!(
        "usage:\n  \
         pocketsmith normalise            scan transactions, stage proposals in payee_normalisations\n  \
         pocketsmith normalise --apply    drain confirmed proposals (write transactions.payee, delete row)\n\n\
         Review and confirm/reject pending proposals through `pocketsmith serve`."
    );
}
...
fn run_scan(conn: &rusqlite::Connection) -> Result<()> {
    ...
    println!("  confirmed: {confirmed}  (run `pocketsmith normalise --apply` to drain)");
